-- Migration: Torna a biometria facial obrigatória no autocadastro público de
-- Funcionários CLT (EmployeeSelfRegistration.jsx), revertendo a permissividade
-- introduzida em 20260913_optional_biometric_in_clt_self_reg.sql para esse
-- fluxo específico.
--
-- O cadastro feito pelo RH/supervisor (FuncionariosTab.jsx, caller_role =
-- 'supervisor') continua ISENTO dessa exigência — o funcionário pode ser
-- lançado no sistema sem biometria e cadastrá-la depois pela tela de
-- Autogestão de Biometria do Quiosque CLT.
...
BEGIN
  ...
  -- Autocadastro público (candidato, sem sessão): biometria facial obrigatória.
  -- O cadastro feito pelo RH/supervisor logado fica isento dessa exigência.
  IF caller_role IS NULL THEN
    IF trim(COALESCE(p_face_descriptor, '')) = '' OR NOT COALESCE(p_biometric_consent_accepted, false) THEN
