...
// quadrante e vê a compatibilidade com o perfil esperado da função.
//
...
function PlacedCard({ candidate, role, busy, onDragStart, onDragEnd, onOpen, onRemove }) {
  ...
  return (
    ...
    >
      <button type="button" onClick={onOpen} className="flex-1 min-w-0 text-left" title="Ver detalhe do encaixe">
        <div className="flex items-center gap-1.5 flex-wrap">
          <span className="text-xs font-semibold text-slate-800 truncate">{candidate.full_name}</span>
          <ProfileBadge assessment={candidate.discAssessment} />
        </div>
        <div className="mt-0.5"><FitBadge fit={fit} /></div>
      </button>
      {busy ? (
